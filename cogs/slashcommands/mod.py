...
class moderation(commands.Cog):
    ...
    async def setlang(self, ctx: discord.Interaction):
        ...
        await ctx.response.send_message(t['args']['lang']['select'], view = discord.ui.View(setlang(self.bot, ctx.author,t)), ephemeral = True)
    ...
    async def setlogs(self, ctx: discord.Interaction):
        ...
        await ctx.response.send_message('',view = discord.ui.View(setlog(self.bot,ctx.author,t)), ephemeral = True)
    ...
    async def kick(self, ctx: discord.Interaction, member: discord.Member, *,reason=None):
        ...
        await ctx.response.send_message(t["args"]["mod"]["confirmkick"].format(member.mention),view = buttonkick(self.bot,member,reason, ctx.author))
    ...
    async def Ban(self, ctx: discord.Interaction, member: discord.Member, *,reason=None):
        ...
        await ctx.response.send_message(t["args"]["mod"]["confirmban"].format(member.mention),view = buttonban(self.bot,member,reason,ctx.author))
    ...
    async def clear(self, ctx: discord.Interaction, qnt: int):
        ...
        if qnt > 1000:
            ...
            await ctx.response.send_message(t["args"]["mod"]["clearchat"].format(len(purge), ctx.author.mention))
    ...
    async def unban(self, ctx: discord.Interaction, id, *, reason = None):
        ...
        description = t["args"]["mod"]["logunban"].format(int(id),ctx.author,reason,int(id)))
    ...
    async def adv(self, ctx: discord.Interaction, member: discord.Member, reason):
        ...
        try:
            ...
            if db['lmod']['True?'] == True:
                ...
                dbmoderation.adcadvdb(ctx.guild,ctx.author,member,f"{t['args']['adv']}{advdb.find_one({ '_id':f'{ctx.guild.id}_{member.id}'})['qnt']}",reason)
