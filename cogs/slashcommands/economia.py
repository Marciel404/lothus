...
class economia(commands.Cog):
    ...
    async def rolar(self, ctx):
        ...
        if rand == 10:
            ...
            dbeconomy.update_bank(ctx.author, + 2000)
            ...
            dbeconomy.update_bank(ctx.author,r)
    ...
    async def Transferir(self, ctx, member: discord.Member, lothcoins: int):
        ...
        if bank.count_documents({"_id": ctx.author.id}) == 0:
            ...
            dbeconomy.update_bank(ctx.author,0)
        ...
        dbeconomy.update_bank(ctx.author,- lothcoins)

        dbeconomy.update_bank(member,+ lothcoins)
    ...
    async def loteria(self, ctx, lothcoins:int):
        ...
        if final[0] == final[1] == final[2]:
            ...
            dbeconomy.update_bank(ctx.author,4*lothcoins)

            await ctx.respond(f'{t["args"]["gan"]} {4*lothcoins} LOTHCOINS!!')

        else:

            dbeconomy.update_bank(ctx.author,-1*lothcoins)
